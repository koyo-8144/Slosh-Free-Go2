...
class LeggedEnv:
    ...
    def _resample_commands_curriculum_linvel_x(self, envs_idx):
        ...
        # tracking_ang_vel_rew_high = self.tracking_ang_vel_rew > (self.tracking_ang_vel_rew_max * self.command_cfg["achieve_rate"])
        
        self.tracking_lin_vel_rew_mean = torch.mean(self.tracking_lin_vel_rew)
        # print("(self.tracking_lin_vel_rew_max * self.command_cfg[achieve_rate]): ", (self.tracking_lin_vel_rew_max * self.command_cfg["achieve_rate"]))
        # print("self.tracking_lin_vel_rew: ", self.tracking_lin_vel_rew)
        ...
        if tracking_lin_vel_rew_high.all():
            ...
            self.lin_vel_x_range_mmax = torch.clip(
                torch.tensor(self.lin_vel_x_range_max + self.command_cfg["increase_rate"]),
                0.0,
                self.command_cfg["lin_vel_x_range_goal"][1]
            ).item()
            # self.lin_vel_y_range_min = torch.clip(self.lin_vel_y_range_min - self.command_cfg["increase_rate"], self.command_cfg["lin_vel_y_range_goal"][0], 0.0)
            # self.lin_vel_y_range_max = torch.clip(self.lin_vel_y_range_max + self.command_cfg["increase_rate"], 0.0, self.command_cfg["lin_vel_y_range_goal"][1])

            self.updated_lin_vel_x_command_range = [self.lin_vel_x_range_min, self.lin_vel_x_range_max]
            # self.updated_lin_vel_y_command_range = [self.lin_vel_y_range_min, self.lin_vel_y_range_max]
        
        # if tracking_ang_vel_rew_high.all():
        #     self.ang_vel_z_range_min = torch.clip(self.ang_vel_z_range_min - self.command_cfg["increase_rate"], self.command_cfg["ang_vel_x_range_goal"][0], 0.0)
        #     self.ang_vel_z_range_max = torch.clip(self.ang_vel_z_range_max + self.command_cfg["increase_rate"], 0.0, self.command_cfg["ang_vel_x_range_goal"][1])

        #     self.updated_ang_vel_z_command_range = [self.ang_vel_z_range_min, self.ang_vel_z_range_max]
    ...
    def give_runner_data(self):
        return self.lin_vel_x_range_min, self.lin_vel_x_range_max, self.tracking_lin_vel_rew_mean, self.tracking_lin_vel_rew_threshold_one
